 ...
 * @author RAMON
 */
public class ButtonEliminarDistritos extends DefaultCellEditor {
	protected JButton button;
	private DeleteButtonListener bListener = new DeleteButtonListener();

	@SuppressWarnings("deprecation")
	public ButtonEliminarDistritos(JCheckBox checkBox) {
		super(checkBox);
            button = new JButton();
	        ...
	        public void actionPerformed(ActionEvent event) {
	        	if(table.getRowCount() > 0){
                            ...
                            if(n==JOptionPane.YES_OPTION){
                                Distrito di=TipoProceso.listaDistritos.get(this.row);
                                TipoProceso.listaDistritos.remove(this.row);
                                if(di.getId()!=0){
